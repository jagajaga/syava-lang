...
pub use llvm_sys::LLVMIntPredicate::{LLVMIntEQ as IntEQ, LLVMIntNE as IntNE, LLVMIntUGT as IntUGT,
                                     LLVMIntUGE as IntUGE, LLVMIntULT as IntULT,
                                     LLVMIntULE as IntULE, LLVMIntSGT as IntSGT,
                                     LLVMIntSGE as IntSGE, LLVMIntSLT as IntSLT,
                                     LLVMIntSLE as IntSLE};

pub use llvm_sys::target_machine::LLVMCodeGenOptLevel::{LLVMCodeGenLevelNone as NoOptimization,
...
impl Value {
    pub fn const_int(ty: Type, value: u64) -> Value {
        unsafe { Value(LLVMConstInt(ty.0, value, false as LLVMBool)) }
    }

    pub fn const_bool(value: bool) -> Value {
        unsafe { Value(LLVMConstInt(LLVMInt1Type(), value as u64, false as LLVMBool)) }
    }
    ...
    pub fn const_struct(values: &[Value]) -> Value {
        unsafe {
            ...
            Value(LLVMConstStruct(llvm_values.as_ptr() as *mut _, len, false as LLVMBool))
        }
    }

    pub fn get_param(func: Value, number: u32) -> Value {
        unsafe { Value(LLVMGetParam(func.0, number)) }
    }
    ...
    fn llvm_slice(value_slice: &[Value]) -> &[LLVMValueRef] {
        ...
        unsafe {
            std::slice::from_raw_parts(value_slice.as_ptr() as *const LLVMValueRef,
                                       value_slice.len())
        }
    }
}
...
impl Builder {
    ...
    pub fn build_call(&self, callee: Value, args: &[Value]) -> Value {
        unsafe {
            ...
            Value(LLVMBuildCall(self.0, callee.0, args.as_ptr() as *mut _, len, cstr!("")))
        }
    }
    ...
    pub fn build_cond_br(&self, cond: Value, then: BasicBlock, else_: BasicBlock) {
        unsafe {
    ...
    pub fn build_load(&self, ptr: Value) -> Value {
        unsafe { Value(LLVMBuildLoad(self.0, ptr.0, cstr!(""))) }
    }
    ...
    pub fn build_neg(&self, inner: Value) -> Value {
        unsafe { Value(LLVMBuildNeg(self.0, inner.0, cstr!(""))) }
    }

    pub fn build_not(&self, inner: Value) -> Value {
        unsafe { Value(LLVMBuildNot(self.0, inner.0, cstr!(""))) }
    }

    pub fn build_add(&self, lhs: Value, rhs: Value) -> Value {
        unsafe { Value(LLVMBuildAdd(self.0, lhs.0, rhs.0, cstr!(""))) }
    }
    pub fn build_sub(&self, lhs: Value, rhs: Value) -> Value {
        unsafe { Value(LLVMBuildSub(self.0, lhs.0, rhs.0, cstr!(""))) }
    }
    pub fn build_mul(&self, lhs: Value, rhs: Value) -> Value {
        unsafe { Value(LLVMBuildMul(self.0, lhs.0, rhs.0, cstr!(""))) }
    }
    pub fn build_udiv(&self, lhs: Value, rhs: Value) -> Value {
        unsafe { Value(LLVMBuildUDiv(self.0, lhs.0, rhs.0, cstr!(""))) }
    }
    pub fn build_sdiv(&self, lhs: Value, rhs: Value) -> Value {
        unsafe { Value(LLVMBuildSDiv(self.0, lhs.0, rhs.0, cstr!(""))) }
    }
    pub fn build_urem(&self, lhs: Value, rhs: Value) -> Value {
        unsafe { Value(LLVMBuildURem(self.0, lhs.0, rhs.0, cstr!(""))) }
    }
    pub fn build_srem(&self, lhs: Value, rhs: Value) -> Value {
        unsafe { Value(LLVMBuildSRem(self.0, lhs.0, rhs.0, cstr!(""))) }
    }

    pub fn build_and(&self, lhs: Value, rhs: Value) -> Value {
        unsafe { Value(LLVMBuildAnd(self.0, lhs.0, rhs.0, cstr!(""))) }
    }
    pub fn build_xor(&self, lhs: Value, rhs: Value) -> Value {
        unsafe { Value(LLVMBuildXor(self.0, lhs.0, rhs.0, cstr!(""))) }
    }
    pub fn build_or(&self, lhs: Value, rhs: Value) -> Value {
        unsafe { Value(LLVMBuildOr(self.0, lhs.0, rhs.0, cstr!(""))) }
    }

    pub fn build_shl(&self, lhs: Value, rhs: Value) -> Value {
        unsafe { Value(LLVMBuildShl(self.0, lhs.0, rhs.0, cstr!(""))) }
    }
    pub fn build_ashr(&self, lhs: Value, rhs: Value) -> Value {
        unsafe { Value(LLVMBuildAShr(self.0, lhs.0, rhs.0, cstr!(""))) }
    }
    pub fn build_lshr(&self, lhs: Value, rhs: Value) -> Value {
        unsafe { Value(LLVMBuildLShr(self.0, lhs.0, rhs.0, cstr!(""))) }
    }

    pub fn build_icmp(&self, pred: LLVMIntPredicate, lhs: Value, rhs: Value) -> Value {
        unsafe { Value(LLVMBuildICmp(self.0, pred, lhs.0, rhs.0, cstr!(""))) }
    }
}
...
impl TargetMachine {
    pub fn new(opt_level: LLVMCodeGenOptLevel) -> Result<Self, TargetMachineError> {
        unsafe {
            ...
            if LLVMGetTargetFromTriple(triple, &mut target, std::ptr::null_mut()) != 0 {
                return Err(TargetMachineError::CouldntGetTargetFromTriple);
            }


            Ok(TargetMachine(LLVMCreateTargetMachine(target,
                                                     triple,
                                                     cstr!(""),
                                                     cstr!(""),
                                                     opt_level,
    ...
    pub fn emit_to_file(&self, module: &Module, output: &str) -> Result<(), String> {
        ...
        unsafe {
            ...
            if LLVMTargetMachineEmitToFile(self.0,
                                           module.0,
...
impl TargetData {
    pub fn from_target_machine(machine: &TargetMachine) -> Self {
        unsafe { TargetData(LLVMGetTargetMachineData(machine.0)) }
    }
}
...
impl Module {
    pub fn new() -> Self {
        unsafe { Module(LLVMModuleCreateWithName(cstr!(""))) }
    }
...
impl FnOptimizer {
    ...
    pub fn optimize(&self, func: Value) {
        unsafe {
            LLVMVerifyFunction(func.0, LLVMVerifierFailureAction::LLVMAbortProcessAction);
...
pub fn get_type(target_data: &TargetData, ty: ty::Type) -> Type {
    ...
    unsafe {
        Type(match *ty.0 {
            TypeVariant::SInt(ref size) | TypeVariant::UInt(ref size) => LLVMIntType(size.size()),
            TypeVariant::Bool => LLVMInt1Type(),
            TypeVariant::Unit => LLVMStructType(std::ptr::null_mut(), 0, false as LLVMBool),
            TypeVariant::Reference(inner) => LLVMPointerType(get_type(target_data, inner).0, 0),
            TypeVariant::Diverging => panic!("ICE: Attempted to get the LLVM type of Diverging"),
...
pub fn get_function_type(target_data: &TargetData, ty: &ty::Function) -> Type {
    unsafe {
        let mut args = ty.input()
                         .iter()
                         .map(|a| get_type(target_data, *a).0)
                         .collect::<Vec<_>>();
